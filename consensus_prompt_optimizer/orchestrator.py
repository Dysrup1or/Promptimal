...
# ============================================================================
# When `--dry-run` is enabled we skip API calls, but we still want a useful,
# deterministic cost estimate for budgeting.
#
# These values represent a typical (not worst-case) output size per stage.
DRY_RUN_ASSUMED_OUTPUT_TOKENS = {
    "discerner": 180,
    "critic_first": 400,
    "expander": 1200,
    "ranker": 80,
...
RANKER_ITERATION_ADDENDUM = """

IMPORTANT: The user has provided a BASELINE prompt that was generated by this tool previously.
Your job is to rank A/B/C by how much they IMPROVE the baseline.

Baseline prompt (preview):
{baseline_preview}

Scoring guidance:
- Prefer candidates that add missing constraints, clarify outputs, add concrete examples, and add self-check steps.
- Penalize candidates that mostly paraphrase the baseline.
- Penalize candidates that add fluff without increasing specificity or usability.
"""
...
SYNTHESIZER_ITERATION_ADDENDUM = """

IMPORTANT: The user is re-submitting a BASELINE prompt (previously generated by this tool).
Your job is to produce a NEW prompt that is measurably better, not a paraphrase.

BASELINE PROMPT (for improvement reference):
{baseline_prompt}

Hard requirements for final_prompt:
1) Must preserve the baseline intent.
2) Must be substantially improved: add missing constraints, clarify output format, add edge cases, and add a self-check.
3) Must avoid fluff; every line should increase usefulness.
4) Do NOT include a changelog or commentary inside final_prompt.
"""


EXPANDER_ITERATION_ADDENDUM = """

IMPORTANT: The user is re-submitting a BASELINE prompt that was generated by this tool.
Generate 3 NEW variations that are STRICTLY better than the baseline.

BASELINE PROMPT:
{baseline_prompt}

Non-negotiable rules:
1) Do NOT paraphrase. Each variation must be structurally different from the baseline.
2) Each variation must introduce at least 5 concrete improvements (constraints, examples, output format, edge cases, self-check).
3) Each variation must remain a META-PROMPT (instructions), not an implementation.
4) Avoid adding extra words that don't add specificity.
"""
...
class PromptimaV2:
    ...
    def run(self, idea: str) -> Dict[str, Any]:
        ...
        # Detect when the user is re-submitting a previously generated Catalyze prompt.
        # In that case, avoid cache hits (which would produce no improvement) and
        # switch into iteration-improvement mode.
        iteration_meta = lookup_generated_prompt(idea)
        iteration_mode = bool(iteration_meta)
        if iteration_mode:
            log_event(
                "pipeline.iteration_mode",
                {
                    "reason": "input_matches_prior_generated_prompt",
                    "ref_idea_hash": (iteration_meta or {}).get("idea_hash"),
                },
            )

        # Check cache first (skip during iteration mode to avoid returning the same output).
        if self.use_cache and not iteration_mode:
        ...
        baseline_prompt = idea if iteration_mode else None
        expansions = self._run_expander(idea, discern, rubric, baseline_prompt=baseline_prompt)
        
        # Stage 4: Ranker
        rankings = self._run_ranker(expansions, rubric, baseline_prompt=baseline_prompt)
        
        # Stage 5: Synthesizer
        final = self._run_synthesizer(idea, discern, rubric, expansions, rankings, baseline_prompt=baseline_prompt)
    ...
    def _run_discerner(self, idea: str) -> DiscernOutput:
        ...
        if self.dry_run:
            self.tracker.record_estimate(
                stage="discerner",
                model=PRIMARY_TEXT_MODEL,
                prompt=prompt,
                max_tokens=500,
                assumed_output_tokens=DRY_RUN_ASSUMED_OUTPUT_TOKENS["discerner"],
            )
    ...
    def _run_critic_first(self, idea: str, discern: DiscernOutput) -> RubricOutput:
        ...
        if self.dry_run:
            self.tracker.record_estimate(
                ...
                prompt=prompt,
                max_tokens=1200,
                assumed_output_tokens=DRY_RUN_ASSUMED_OUTPUT_TOKENS["critic_first"],
            )
            return RubricOutput(
    ...
    def _run_expander(
        ...
        *,
        ...
        if baseline_prompt:
            prompt = prompt + EXPANDER_ITERATION_ADDENDUM.format(
                baseline_prompt=baseline_prompt[:4000]
            )
        ...
        if self.dry_run:
            self.tracker.record_estimate(
                ...
                assumed_output_tokens=min(
                    GROQ_TOKEN_CAP,
                    DRY_RUN_ASSUMED_OUTPUT_TOKENS["expander"],
                ),
            )
...
CRITICAL: Start your response with {{ and end with }}. No other text."""
    ...
    def _run_ranker(
        ...
        *,
        ...
        variations_summary = {
            "A": {"notes": expansions.A.notes, "checklist": expansions.A.checklist_score, "preview": expansions.A.prompt[:350]},
            "B": {"notes": expansions.B.notes, "checklist": expansions.B.checklist_score, "preview": expansions.B.prompt[:350]},
            "C": {"notes": expansions.C.notes, "checklist": expansions.C.checklist_score, "preview": expansions.C.prompt[:350]}
        }
        ...
        if baseline_prompt:
            prompt = prompt + RANKER_ITERATION_ADDENDUM.format(
                baseline_preview=baseline_prompt[:350]
            )
        ...
        if self.dry_run:
            self.tracker.record_estimate(
                ...
                prompt=prompt,
                max_tokens=150,
                assumed_output_tokens=DRY_RUN_ASSUMED_OUTPUT_TOKENS["ranker"],
            )
            ...
            raw = parse_json_response_v2(response["content"])
            rankings = validate_stage_output(raw, RankingsOutput)

        # If we have a baseline prompt, apply a deterministic novelty penalty.
        # This prevents "same quality" paraphrase loops on re-iteration.
        if baseline_prompt:
            import difflib
            ...
            candidates = {
                "A": expansions.A.prompt,
                "B": expansions.B.prompt,
                "C": expansions.C.prompt,
            }

            adjusted_scores: Dict[str, float] = {}
            for key, cand in candidates.items():
                sim = similarity(base, cand)
                # Penalize high similarity. No penalty up to ~0.75; steep penalty above.
                penalty = max(0.0, (sim - 0.75) * 2.0)
                original_score = float(getattr(rankings, key).score)
                adjusted_scores[key] = max(0.0, min(1.0, original_score - penalty))

            # Re-rank based on adjusted score (desc). Tie-break on original rank.
            original_ranks = {
                "A": int(rankings.A.rank),
                "B": int(rankings.B.rank),
                "C": int(rankings.C.rank),
            }
            order = sorted(
                ["A", "B", "C"],
                key=lambda k: (-adjusted_scores[k], original_ranks[k]),
            )
            new_ranks = {order[0]: 1, order[1]: 2, order[2]: 3}

            rankings = RankingsOutput(
                A=RankerVariant(rank=new_ranks["A"], score=adjusted_scores["A"]),
                B=RankerVariant(rank=new_ranks["B"], score=adjusted_scores["B"]),
                C=RankerVariant(rank=new_ranks["C"], score=adjusted_scores["C"]),
            )
    ...
    def _run_synthesizer(
        ...
        *,
        ...
        if baseline_prompt:
            prompt = prompt + SYNTHESIZER_ITERATION_ADDENDUM.format(
                baseline_prompt=baseline_prompt[:6000]
            )
        ...
        if self.dry_run:
            self.tracker.record_estimate(
                ...
                prompt=prompt,
                max_tokens=2000,
                assumed_output_tokens=DRY_RUN_ASSUMED_OUTPUT_TOKENS["synthesizer"],
            )
